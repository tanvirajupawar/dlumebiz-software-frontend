...
const CreatePurchaseReturn = () => {
  ...
  const [returnNumber, setReturnNumber] = useState("PR-001");
  ...
  useEffect(() => {
  axios.get("http://localhost:8000/api/purchase-return/next-number")
    .then(res => {
      if (res.data.success) {
    ...
    .catch(() => {
      setReturnNumber("PR-001"); // fallback
    });
}, []);
...
<div className="grid grid-cols-3 gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  {invoice ? new Date(invoice.invoice_date).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" }) : "—"}
                </p>
              </div>
            </div>
          <div>
