...
const CustomerSelector = ({ value, onChange }) => {
  ...
  useEffect(() => {
   axios.get("http://localhost:8000/api/customers")
      ...
      .catch((err) => { console.error("CUSTOMER FETCH ERROR:", err); setCustomers([]); });
  }, []);
    ...
    <div className="relative">
      <button type="button" onClick={() => setOpen((o) => !o)}
        ...
        <span className={value ? "font-medium text-gray-800" : ""}>
          {value ? (value.company_name || `${value.first_name} ${value.last_name}`) : "+ Select Customer"}
        </span>
              ...
              {filtered.length === 0
                ...
                : filtered.map((c) => (
                  <button key={c._id} onClick={() => select(c)}
                    ...
                    <div>
                      <p className="text-sm font-medium text-gray-800">{`${c.first_name || ""} ${c.last_name || ""}`.trim() || c.company_name}</p>
        ...
        <div className="flex justify-end gap-6 pb-6">
          <div className="w-full max-w-sm">
            ...
            <div className="flex gap-3">
              <button onClick={() => navigate("/credit-note")}
